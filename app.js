...
function familyVoteLabel(itemId) {
  const votes = familyCount(itemId);
  return `${votes} de ${CONFIG.familySize || 4} interesado${votes === 1 ? '' : 's'}`;
}

function compareByFamilyVotes(a, b) {
  return familyCount(b.id) - familyCount(a.id)
...
function appHeader() {
  const title = state.view === 'detail' ? 'Ficha completa' : state.view === 'decide' ? '¿Qué hacemos ahora?' : state.view === 'catalog' ? 'Catálogo de Nueva York' : state.view === 'today' ? 'Agenda de actividades' : 'Plan familiar';
  ...
  <nav class="primary-nav" aria-label="Navegación principal">${[['decide','D','Decidir'],['catalog','C','Catálogo'],['today','A','Agenda'],['plan','P','Plan']].map(([id,line,label]) => `<button class="nav-button ${state.view === id ? 'active' : ''}" data-view="${id}"><span class="nav-line">${line}</span>${label}</button>`).join('')}</nav>`;
}
...
function inferredTypeLabel(item) {
  if (item.itemKind === 'activity') return activityLabel(item.category || item.type);
  if (item.origin !== 'family' || !isUnclassifiedFamilyProposal(item)) return item.type || item.itemKind;
...
function activityType(item) {
  return item.category || item.type || 'Actividad';
}
...
function decideMatches() {
  ...
  return source.filter(item => {
    ...
    if (d.familyInterest === 'any' && familyCount(item.id) < 1) return false;
    if (d.familyInterest === 'shared' && familyCount(item.id) < 2) return false;
    return true;
  }).map(item => {
    const priority = { alta: 3, media: 2, baja: 1 }[normalize(item.priority)] || 1;
  ...
  }).sort((a, b) => d.near && state.userLocation
    ? (a.distanceKm ?? 9999) - (b.distanceKm ?? 9999) || familyCount(b.id) - familyCount(a.id) || b.score - a.score
    : familyCount(b.id) - familyCount(a.id) || b.score - a.score || (b.rating || 0) - (a.rating || 0));
}
...
function renderToday() {
  if (state.loadingCatalog) {
    return `<section class="view active"><div class="section-head"><div><h2>Agenda de actividades</h2><p class="muted">Cargando calendario de actividades…</p></div></div><div class="panel empty loading-panel"><h3>Sincronizando con Google Sheets</h3><p class="muted">La agenda aparecerá aquí en cuanto llegue el snapshot.</p></div></section>`;
  }
  ...
  return `<section class="view active"><div class="section-head"><div><h2>Agenda de actividades</h2><p class="muted">${state.activities.length} actividades en calendario · ${modeLabel}</p></div></div>
